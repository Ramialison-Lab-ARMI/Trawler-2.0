...
# $Id: pipeline_trawler_02.pl,v 1.15 2009/04/27 11:49:10 haudry Exp $
    ...
    if ($$id2specie2sequences{$id}->{$ref_species}) { #put back to "reference"
        ...
        foreach my $idmotif (keys %$id2motif) {
            ...
            foreach my $motif (@motifs) {
                ...
                my ($mot, $mstarts, $mends, $strs) = get_motif_loc($ref_seq,$motif);
                if($mot) {
                    my $size_mot = @$mot;
...
sub get_motif_positions {
    ...
    my $total_real = 0;
    my ($motifs, $starts, $ends, $strs) = get_motif_loc($conserved_seq, $motif);
    my $size = @$motifs;
    for(my $i=0; $i< $size; $i++) {
        my $start = $$starts[$i];
        my $end = $$ends[$i];
...
sub get_motif_loc {
    ...
    print "motif $motif $pattern_matching1 $pattern_matching2 \n" if $DEBUG;
    my $matches = 0;
    ...
    while ($DNA =~ m/($pattern_matching2)/g) {
        my $m = $1;
        my $motif_length = length($m);
        my $pos = pos $DNA;
        push @start, ($pos-$motif_length);
...
sub get_stat_motif {
    ...
    for(my $i=0; $i< $size; $i++) {
        ...
        print STAT "$id\t$motif_id\t$motif\t$start\t$average_conservation\t$conserved_count\t$size_seq\t$strand\t$start_from_end\n";
