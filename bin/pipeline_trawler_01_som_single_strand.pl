...
my $class; #either vertebrate, fungi, plant, insect or nematode for treg comp
...
if ($nb_of_cluster eq "0"){
    print STDERR "performing SCC clustering with -$nb_of_cluster-\n";
    ...
    print STDERR "performing kmean clustering with -$nb_of_cluster-\n";
...
my $command_TREG1 = "\"" . $tcst{TREG_COMP} . "\" -q \"$pwm_file\" -f raw -d \"" . $tcst{TREG_MATRIX} . "\" -g $class -ori both -c $treg_threshold -out \"$compare_file\"";
